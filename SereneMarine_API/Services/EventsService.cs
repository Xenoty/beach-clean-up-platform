...
namespace WebApi.Services
{
    ...
    public class EventsService : IEventService
    {
        ...
        public List<Event> GetAll() =>
         _events.Find(ev => true).ToList();

        public Event GetById(string id)
        {
            //also need to get current attendance for event
            var temp = _events.Find<Event>(ev => ev.event_id == id).FirstOrDefault();

            temp.current_attendance = _ea.Find<EventAttendance>(ea => ea.event_id == id).ToList().Count();
        ...
        {
            var ev = _events.Find<Event>(ev => ev.event_id == eventParam.event_id).SingleOrDefault();
            ...
            if (!string.IsNullOrWhiteSpace(eventParam.event_name) && eventParam.event_name != ev.event_name)
            {
                ...
                if (_events.Find<Event>(x => x.event_name == eventParam.event_name).FirstOrDefault() != null)
        ...
        {
            var ev = _events.Find<Event>(ev => ev.event_id == id).FirstOrDefault();
            if (ev != null)
            {
